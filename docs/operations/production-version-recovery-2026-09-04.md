...
`0021` 会保留线上已有模型通道，只在缺少表/字段时创建或补齐，同时补齐 `assistant_image_prompt`。首次从单通道配置升级时会直接复制现有主密钥的密文、IV 和 auth tag，不会把密钥解密回 SQL 明文。SQL 迁移完成后仍须运行通道密钥迁移脚本，以兼容历史上可能存在的明文渠道；脚本在单个数据库事务中处理所有通道，只有成功生成密文后才清空旧明文，重复执行不会重复加密。`0023` 为幂等记录增加不可为空的租约令牌，并把升级时仍在处理的旧请求封存为 409，避免新旧 worker 同时执行同一个请求；数据库默认生成租约令牌，确保必要时切回旧 release 后旧代码仍能插入记录。
